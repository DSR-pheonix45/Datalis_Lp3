...
export function AuthProvider({ children }) {
  ...
  const signup = async (email, password, username) => {
    try {
      ...
      // If signup was successful but we're using Supabase (not offline mode)
      if (result.success && result.user) {
        if (result.user.id.startsWith('local_')) {
          // This is an offline user, log them in directly
          setUser(result.user);
          localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(result.user));
        } else {
          // This is a Supabase user, they need to verify email first
          // Don't set the user yet
          return { 
            success: true, 
